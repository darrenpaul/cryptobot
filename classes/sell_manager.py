...
class SellManager:
    ...
    def _create_sell_order(self, price, quantity):
        simple_order = luno.create_sell_order(
            self.trading_pair, price, quantity, dry_run=self.dry_run
        )
    ...
    def process_pending_sell_orders(self):
        updated_pending_orders = order_utils.update_order_details(
            self.pending_orders_sell
        )
    ...
    def check_for_possible_sell_orders(self, current_price):
        ...
        if len(orders) > 0:
            weighted_price = mathematics.get_weighted_average(
                orders, "limit_price", "quantity"
            )
            sell_price = self._get_sell_price(weighted_price, current_price)
            self.process_possible_sell_orders(
                sell_price, total_quantity, orders, order_ids
            )
